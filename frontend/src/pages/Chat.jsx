...
function Chat({ userData, socket }) {
  ...
  // 1. JOIN ROOM ON LOAD (Handle Refresh)
  useEffect(() => {
    if (userData && roomId) {
       // Re-join the room immediately
  ...
  }, [messageList, typingUser, uploading]);
    ...
    <div className="flex w-full max-w-5xl h-[90vh] bg-[#0b141a] border border-gray-700 rounded-lg overflow-hidden shadow-2xl relative mx-auto mt-5">
      ...
      <div className="flex-1 flex flex-col bg-[#0b141a] relative bg-[url('https://user-images.githubusercontent.com/15075759/28719144-86dc0f70-73b1-11e7-911d-60d70fcded21.png')] bg-repeat">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
            {messageList.map((msg, index) => {
              ...
              return (
                <div key={index} className={`flex w-full ${isMyMessage ? "justify-end" : "justify-start"}`}>
                    ...
                    <div className={`max-w-[65%] min-w-[120px] px-3 py-2 rounded-lg text-sm shadow-md relative group ${isMyMessage ? "bg-[#005c4b] text-white" : "bg-[#202c33] text-white"}`}>
            ...
            {uploading && <div className="text-right text-green-500 text-xs">Uploading...</div>}
            <div ref={bottomRef} />
        </div>
        ...
        <div className="bg-[#202c33] p-2 flex gap-2 items-center z-10">
            <button onClick={() => setShowEmoji(!showEmoji)} className="text-2xl text-gray-400 p-2">😊</button>
            <button onClick={() => fileInputRef.current.click()} className="text-2xl text-gray-400 p-2">📎</button>
            ...
            <input type="text" value={currentMessage} placeholder="Type a message..." className="flex-1 p-3 bg-[#2a3942] text-white rounded-lg outline-none"
                onChange={handleTyping} onKeyPress={(e) => { e.key === "Enter" && sendMessage(); }} />
            <button onClick={sendMessage} className="bg-[#00a884] p-3 rounded-full text-white">➤</button>
        </div>
      </div>
    </div>
  );
